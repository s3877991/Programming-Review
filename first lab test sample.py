...
def find_top3(numbers):
    ...
    number_list = []
    sorted_number_list = []
    for string_element in number_string_list:        # Convert a number string element to integer in a list
        number_element = int(string_element)
        number_list.append(number_element)
    while number_list:                              # Sort the element from minimum to maximum values
    ...
    max_index = len(sorted_number_list)                                     # Find 3 maximum values at the end of the
    top_3_list = sorted_number_list[max_index - 3: max_index + 1]           # sorted list
    return top_3_list
...
def outcome():
    ...
    if n == '1':
        ...
        for side in range(3, 11):
            multiple_color = color * side           # There are 8 shapes to color but there are 4 colors to fill shapes
